...
schema	= 	str(sys.argv[13]) 
output	= 	str(sys.argv[14]) 
...
reco 	= 	t24.join(t13,cond,"Inner").select(t24.cdsinies, t24.pocoaseg, t13.cdfiliac,F.when((t13.cdfiliac == '0002103504') & (F.trim(t24.inticoas ) == 'C'), m13.popacoac).otherwise(0).alias("porc_para_tomado"),
...
SinCob=SinCob.groupBy(
						SinCob.cve_siniestro.alias("cve_siniestro1"),								SinCob.cve_cobertura_tarificable.alias("cve_cobertura_tarificable1"),																	
						...
						F.max(SinCob.desc_causa_siniestro).alias("desc_causa_siniestro"), 						
						F.max(SinCob.feocusin_fch).alias("feocusin_fch"),										F.max(SinCob.hora_ocurrencia_siniestro).alias("hora_ocurrencia_siniestro"), 
						...
						F.max(SinCob.desc_cobertura_afecta).alias("desc_cobertura_afecta"),						F.max(SinCob.cve_cobertura_afecta_agrup).alias("cve_cobertura_afecta_agrup"),
					 	F.max(SinCob.operador_reserva).alias("operador_reserva"),								
																												F.max(SinCob.fecomuni_fch).alias("fecomuni_fch"),
...
aut4 	= 	aut4.groupBy(aut4.cve_siniestro.alias("cve_siniestro5"),aut4.num_afectado.alias("num_afectado5")).agg(F.max(aut4.cve_tipo_vehiculo).alias("cve_tipo_vehiculo_tercero"),F.max(aut4.descr_tipo_vehiculo).alias("desc_tipo_vehiculo_tercero"),
			...
			F.trim(F.max(aut4.carroceria)).alias("carroceria"),F.max(aut4.descripcion_vehiculo).alias("desc_vehiculo_tercero"),F.max(aut4.clave_armadora).alias("cve_armadora_tercero"),F.max(aut4.descr_armadora).alias("desc_armadora_tercero"),F.max(aut4.cve_uso_vehiculo).alias("cve_uso_vehiculo_tercero"),
...
def printSalvamentos (x):
	...
	if x["mto_mov_ori"] != 0 and x["porc_para_tomado"] > 0:
		if  x["cve_tipo_mov"] in ('1','2'):
			mto_sin_tomado_ori = float(x["mto_mov_ori"] * -1) * float(x["porc_para_tomado"]/100)
		else: 
			mto_sin_tomado_ori = float(x["mto_mov_ori"]) * float(x["porc_para_tomado"]/100) 	
			
	mto_sin_tomado_int = 0.0
	if x["mto_mov_int"] != 0 and x["porc_para_tomado"] > 0:
		if  x["cve_tipo_mov"] in ('1','2'):
			mto_sin_tomado_int = float(x["mto_mov_int"] * -1) * float(x["porc_para_tomado"]/100)
		else:
			mto_sin_tomado_int = float(x["mto_mov_int"]) * float(x["porc_para_tomado"]/100) 
...
#g.show(10)
#print f.take(10)
g.write.format("parquet").mode("overwrite").saveAsTable(schema+"."+output)