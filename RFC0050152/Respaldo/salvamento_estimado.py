...
conf2 = pyspark.SparkConf().setAll([("spark.sql.legacy.allowCreatingManagedTableUsingNonemptyLocation","true"),("spark.sql.session.timeZone", "GTM")])
spark_context = SparkContext.getOrCreate(conf=conf2)
...
OUTPUT_TABLE_NAME7 = str(sys.argv[10])#salvamento_estimado
...
def loadUniverse(name):
		t = hive_context.table(name+"t").drop(col("tscdc"))
...
).filter(col("CDPRODTE").like('A%')).distinct()
...
def imCalc (y):
	...
	if y["tccdmone"] == "MXP":
		rate_mult = 1.0
	elif y["tccdmone"] == "USD" and y["rate_mult2"] == None:
		rate_mult = 0.0
...
#g.show()
g.write.format("parquet").mode("overwrite").saveAsTable(SCHEMA_DES+"."+OUTPUT_TABLE_NAME7)
